...
def get_asg(aws_region: str) -> str:
    """
    ...
    return client.describe_auto_scaling_groups(
        Filters=[
            {
                'Name': f"tag:{APPLICATION_NAME_KEY}",
                'Values': [APPLICATION_NAME_VALUE]
            },
        ]
    )['AutoScalingGroups'][0]


def update_asg_capacity(asg, region: str, desired_capacity: int) -> int:
    """
    ...
    if desired_capacity != current_capacity:
        logger.debug("Updating ASG capacity to %s in region %s", desired_capacity, region)
        client.update_auto_scaling_group(
            AutoScalingGroupName=asg['AutoScalingGroupName'],
            DesiredCapacity=desired_capacity
        )
    else:
        logger.debug("ASG capacity is already %s in region %s", desired_capacity, region)
    return desired_capacity


def _get_instance_public_ip(asg, region) -> str:
...
def get_instance_from_asg(asg, region: str) -> str:
    """Gets the public IP address of the instance."""
...
def update_security_group(asg, allowed_client_ip, region_name):
    """
    ...
    security_group = ec2.describe_security_groups(
        GroupIds=[security_group_id])["SecurityGroups"][0]
    ...
    if (len(permissions) > 0 and len(authorize_permissions) > 0
...
def set_dns_alias(alias_name: str, hosted_zone_name, asg: str, region: str):
    """
    ...
    hosted_zone_id = client.list_hosted_zones_by_name(
        DNSName=hosted_zone_name)["HostedZones"][0]["Id"]
    for r in client.list_resource_record_sets(HostedZoneId=hosted_zone_id)["ResourceRecordSets"]:
